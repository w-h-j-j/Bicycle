...
import com.example.bicycle.databinding.ActivityMusicPlayerBinding;
...
public class MusicPlayerActivity extends AppCompatActivity {
    ...
    private void initRecyclerView() {
        ...
        binding.recyclerMusicList.setLayoutManager(new LinearLayoutManager(this));
            ...
            if (cursor != null) {
                ...
                while (cursor.moveToNext()) {
                    ...
                    if (title != null && !title.isEmpty()) {
                        ...
                        musicList.add(music);
                        XLog.d("发现音乐: " + title + " - " + artist);
                    }
                }
                cursor.close();

                XLog.d("总共找到 " + musicList.size() + " 首歌曲");

                runOnUiThread(() -> {
                    musicAdapter.notifyDataSetChanged();
                    binding.tvMusicCount.setText(musicList.size() + " 首歌曲");
                    if (musicList.isEmpty()) {
                        XLog.w("未找到任何音乐文件，请确保：\n1. 已授予存储权限\n2. 设备上有音乐文件\n3. 音乐文件在可扫描的目录中");
                    }
                });
            } else {
                XLog.e("Cursor 为 null，无法查询音乐文件");
                runOnUiThread(() -> {
                    binding.tvMusicCount.setText("无法访问音乐库");
                });
            }
        }).start();
    }
            ...
            progressRunnable = new Runnable() {
                ...
                public void run() {
                    if (isBound && musicService != null) {
                        ...
                        // 如果播放完成，更新 UI
                        if (currentProgress >= duration - 100 && duration > 0) {
                            updatePlayingUI();
                        }
                    }
